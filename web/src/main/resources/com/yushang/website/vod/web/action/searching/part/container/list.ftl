          ...
          <nav aria-label="Page navigation" class="text-right">
            <ul class="pagination pagination-sm">
              <li[#if videos.pageIndex lte 1] class="disabled"[/#if]>
                <a href="[#if videos.pageIndex lte 1]javascript:void(0)[#else]${base}/searching/${Parameters["key"]}?pageNo=${videos.pageIndex - 1}[/#if]" aria-label="Previous">
              ...
              [/#if]
              [#list start..end as i]
              <li[#if i == videos.pageIndex] class="active"[/#if]><a href="${base}/searching/${Parameters["key"]}?pageNo=${i}">${i}</a></li>
              [/#list]
              <li[#if videos.pageIndex gte videos.totalPages] class="disabled"[/#if]>
                <a href="[#if videos.pageIndex gte videos.totalPages]javascript:void(0)"[#else]${base}/searching/${Parameters["key"]}?pageNo=${videos.pageIndex + 1}[/#if]" aria-label="Next">
