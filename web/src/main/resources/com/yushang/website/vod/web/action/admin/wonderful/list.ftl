[@b.head/]
  [@b.grid items=wonderfuls var="wonderful"]
    ...
    [@b.row]
      ...
      [@b.col title="视频时长" property="video.second"][#assign s = wonderful.video.second/]${(s / 60)?int}分钟${(s % 60 == 0)?string("", (s / 60 - (s / 60)?int) * 60) + "秒"}[/@]
      ...
      [@b.col title="视频" sortable="false"]
        <video src="${base}/admin/video/video/${wonderful.video.id}" width="85" height="85" controls="controls" title="${wonderful.video.videoName}"/>
      [/@]
      --]
