[@b.head/]
  [@b.toolbar title="精彩视频-添加-<span style=\"color:blue\">第2步：配置已选择的视频</span>"]
    bar.addItem("返回", function () {
      bg.form.submit("searchForm");
    }, "action-backward");
  [/@]
  [@b.form name="wonderfulForm" action="!save" target="wonderfuls" theme="list"]
  ...
  <script>
    $(function() {
      $(document).ready(function() {
        $(".grid").appendTo($("form#wonderfulForm"));
        $("fieldset").remove();
      });
    });
  </script>
